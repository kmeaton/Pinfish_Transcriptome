...
goinfo="niletilapia_gene_GO_nodups_5june21.csv" # the name of a file containing genes and GO term info from your organism that you BLASTed against. We can get this from ENSEMBL, I will show you
...
cat $blastfile | awk -v mincov="$mincoverage" '$13 > mincov {print $0}' | awk -v minid="$minpercentid" '$3 >= minid {print $0}' | awk -v ev="$evalue" '$11 < ev {print $0}' > tilapia_multiple_matches.txt
...
cat tilapia_multiple_matches.txt | awk '{print $1}' | sort | uniq > tilapia_annotated_transcripts.txt
...
for transcript in `cat tilapia_annotated_transcripts.txt`
do
    egrep "$transcript\s" tilapia_multiple_matches.txt > temp.txt
    sort -nr -k 12 temp.txt > sorted.txt
    head -1 sorted.txt >> tilapia_one_annotation_per_transcript.txt
    rm temp.txt
    rm sorted.txt
done
...
cat tilapia_one_annotation_per_transcript.txt | awk '{print $1,$2,$3,$11,$12,$13}' | sed -r 's/\s+/,/g' > tilapia_ids.txt
...
for gene in `cat tilapia_ids.txt`
do
	...
	grep "$EI" $goinfo | sed "s/^ENS/$TRINITY,$PID,$EVAL,$BIT,$COV,ENS/g" >> tilapia_annotation_table.csv
done
...
seqkit grep -v -f tilapia_annotated_transcripts.txt croaker_unannotated_contigs.fa -o tilapia_unannotated_contigs.fa
