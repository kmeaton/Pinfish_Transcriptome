...
cat $blastfile | awk -v mincov="$mincoverage" '$13 > mincov {print $0}' | awk -v minid="$minpercentid" '$3 >= minid {print $0}' | awk -v ev="$evalue" '$11 < ev {print $0}' > uniprot_trembl_multiple_matches.txt
...
cat uniprot_trembl_multiple_matches.txt | awk '{print $1}' | sort | uniq > uniprot_trembl_annotated_transcripts.txt
...
for transcript in `cat uniprot_trembl_annotated_transcripts.txt`
do
    egrep "$transcript\s" uniprot_trembl_multiple_matches.txt > temp.txt
    sort -nr -k 12 temp.txt > sorted.txt
    head -1 sorted.txt >> uniprot_trembl_one_annotation_per_transcript.txt
    rm temp.txt
    rm sorted.txt
done
...
cat uniprot_trembl_one_annotation_per_transcript.txt | awk '{print $1,$2,$3,$11,$12,$13}' | sed -r 's/\s+/,/g' > uniprot_trembl_ids.txt
