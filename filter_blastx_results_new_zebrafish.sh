...
blastfile="./zebrafish_full_gene.OUT" # the name of your output table from BLAST
goinfo="zebrafish_gene_GO_nodups_5june21.csv" # the name of a file containing genes and GO term info from your organism that you BLASTed against. We can get this from ENSEMBL, I will show you
...
do
	...
	grep "$EI" $goinfo | sed "s/^ENS/$TRINITY,$PID,$EVAL,$BIT,$COV,ENS/g" >> zebrafish_annotation_table.csv
done
...
seqkit grep -v -f zebrafish_annotated_transcripts.txt tilapia_unannotated_contigs.fa -o zebrafish_unannotated_contigs.fa
